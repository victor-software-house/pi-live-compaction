...
describe('custom liquid filters', () => {
	it('text filter pulls plain text from a Pi Message', async () => {
		const out = await withTemplate('{{ kept_tail_messages | first | text }}');
		expect(out).toBe('now do X');
	});

	it('last_user_text returns the newest user text across groups', async () => {
		const out = await withTemplate('{{ kept_tail_messages | last_user_text: discarded_messages }}');
		expect(out).toBe('now do X');
	});
	...
	it('quote escapes `"` and trims', async () => {
		const out = await withTemplate('{{ "  he said \\"hi\\"  " | quote }}');
		expect(out).toBe('"he said \\"hi\\""');
	});

	it('present is true for non-empty strings, false for empty/missing', async () => {
		expect(await withTemplate('{% if discarded | present %}yes{% else %}no{% endif %}')).toBe(
			'yes',
		);
		expect(await withTemplate('{% if focus | present %}yes{% else %}no{% endif %}')).toBe('no');
	});
});

describe('{% xml %} block tag', () => {
	it('wraps the body in <tag>…</tag>', async () => {
		const out = await withTemplate('{% xml "focus" %}body content{% endxml %}');
		expect(out).toBe('<focus>\nbody content\n</focus>');
	});

	it('emits nothing when the body renders empty', async () => {
		const out = await withTemplate('{% xml "focus" %}{{ focus }}{% endxml %}');
		expect(out).toBe('');
	});
	...
	it('rejects malformed tag arguments at parse time', async () => {
		...
		const templatePath = path.join(dir, 'compaction-prompt.md');
		await writeFile(templatePath, '{% xml not-quoted %}body{% endxml %}', 'utf8');
		await expect(loadCompactionTemplate(templatePath)).rejects.toThrow(/xml/);
	});
});
