...
describe('config defaults', () => {
	...
	it('rejects invalid defaultPanelScope values', () => {
		expect(() => parseConfig({ defaultPanelScope: 'nope' })).toThrowError(/defaultPanelScope/);
		expect(() => parseConfig({ defaultPanelScope: 5 })).toThrowError(/defaultPanelScope/);
	});

	it('accepts inheritSessionModel boolean and rejects non-booleans', () => {
		expect(parseConfig({ inheritSessionModel: true }).inheritSessionModel).toBe(true);
		expect(parseConfig({ inheritSessionModel: false }).inheritSessionModel).toBe(false);
		expect(() => parseConfig({ inheritSessionModel: 'yes' })).toThrowError(/inheritSessionModel/);
	});
});
