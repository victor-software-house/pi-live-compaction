...
describe("SYSTEM_PROMPT", () => {
	it("keeps non-overridable carry-forward invariants in system scope", () => {
		expect(SYSTEM_PROMPT).toContain("will not see `<previous-summary>`, `<discarded-conversation>`, `<files-touched>`, or `<focus>` as separate blocks");
		...
		expect(SYSTEM_PROMPT).toContain("Preserve user messages from `<discarded-conversation>` with higher fidelity");
