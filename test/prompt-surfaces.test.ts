...
import { fetchTaskStateSnapshot, runLiveCompaction } from "../extensions/live-compaction/index";
...
const filesTouchedBlock = renderFilesTouchedManifestBlock([
	{ path: path.join(packageRoot, "README.md"), displayPath: "README.md", operations: new Set(["read"]), lastTimestamp: 1000 },
]);
...
describe("prompt/template surfaces", () => {
	...
	it.skipIf(!hasLocalExtension)("renders live custom global compaction template with package partials", async () => {
		const templatePath = path.join(agentDir, "extensions/grounded-compaction/compaction-prompt.md");
	...
	it.skipIf(!hasLocalExtension)("renders live custom global branch-summary prompt", async () => {
		const templatePath = path.join(agentDir, "extensions/grounded-compaction/branch-summary-prompt.md");
		expect(existsSync(templatePath), templatePath).toBe(true);
	...
	it("renders packaged compaction prompt example directly", async () => {
		const templatePath = path.join(packageRoot, "extensions/live-compaction/compaction-prompt.md.example");
		const out = await renderCompactionTemplate(templatePath);
	...
	it("renders packaged branch-summary prompt example directly", async () => {
		const templatePath = path.join(packageRoot, "extensions/live-compaction/branch-summary-prompt.md.example");
		const out = await renderBranchSummaryTemplate(templatePath);

		expectValidRenderedPrompt(out);
		expect(out).toContain("## Purpose");
		expect(out).toContain("## User intent & branch delta");
	});

	it("keeps carry-forward invariants in system prompt even with a custom minimal template", async () => {
		let capturedSystemPrompt = "";
		let capturedPromptBody = "";
		const customTemplate = loadCompactionTemplateFromString("CUSTOM BODY ONLY: {{ focus }}\nTASKS: {{ task_state }}", {
		...
		await runLiveCompaction(
			...
			} as never,
			{
				...
				resolvePaths: () => ({ global: { compactionPromptPath: "/tmp/custom.md" }, project: undefined }),
				fetchTaskState: async () => taskStateBlock,
			} as never,
		);

		expect(capturedSystemPrompt).toBe(SYSTEM_PROMPT);
		expect(capturedSystemPrompt).toContain("will not see `<previous-summary>`, `<discarded-conversation>`, `<files-touched>`, or `<focus>` as separate blocks");
		...
		expect(capturedPromptBody).toContain("#28 [in_progress] Anchor compaction with live task state");
