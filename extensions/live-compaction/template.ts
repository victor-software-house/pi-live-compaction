...
function asCompactionTemplate(
	loaded: LoadedTemplate<Record<string, unknown>>,
): CompactionTemplate {
	return {
...
function toCompactionTemplateError(
	...
	const templatePath =
		error instanceof TemplateKitError
			? error.templatePath
		...
		return asCompactionTemplate(
			await loadTemplate(templatePath, {
				parseFrontmatter: (raw) =>
					parseFrontmatter(raw) as unknown as Record<string, unknown>,
			}),
		);
	} catch (error) {
...
export function buildRenderVars(
	options: BuildRenderVarsOptions,
): CompactionRenderVars {
	const lastUserMessage = findLastUserText([
		options.keptTailMessages,
		options.discardedMessages,
	]);
