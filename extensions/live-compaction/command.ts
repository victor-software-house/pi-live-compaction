		...
		},
		{
			...
			submenu: () =>
				createSelectionSubmenu(
					...
					requestRender,
					(selectedValue) => {
						if (selectedValue === 'add' || selectedValue === 'edit' || selectedValue === 'delete') {
...
function syncPanelItems(items: SettingItem[], state: LiveCompactionState): void {
...
async function pickInitialScope(ctx: ExtensionCommandContext): Promise<ConfigScope> {
			...
			settingsList = new SettingsList(
				...
				async (id, newValue) => {
					...
					if (id === SETTING_IDS.defaultPreset) {
						await controller.setConfig(scope, { ...state.config, defaultPreset: newValue }, ctx);
						await refreshState();
					}
				},
				() => done(undefined),
