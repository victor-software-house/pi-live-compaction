...
function describePromptSource(kind: PromptKind, prompt: PromptResolution): string {
	if (kind === 'compaction') {
		if (prompt.source === 'default') return 'compactionPrompt=default';
		if (prompt.isBlankOverride) return `compactionPrompt=${prompt.source}:blank->default`;
		return `compactionPrompt=${prompt.source}:custom`;
	}
	if (prompt.source === 'default') return 'branchPrompt=default';
	if (prompt.isBlankOverride) return `branchPrompt=${prompt.source}:blank->disabled`;
...
function getEffectiveScope(state: LiveCompactionState): ConfigScope | 'default' {
	if (state.scope === 'project' || !state.projectScopeAvailable) return state.scope;
	if (state.projectScopeHasOverrides) return 'project';
	return 'global';
}
...
export function createLiveCompactionController(): LiveCompactionController {
	...
	return {
		...
		async deletePreset(scope, name, ctx) {
			...
			if (config.defaultPreset === name) config.defaultPreset = CURRENT_PRESET_SENTINEL;
			await this.setConfig(scope, config, ctx);
		},
		...
		async refreshRuntimeStatus(ctx) {
			...
			runtimeStatus = { available: issues.length === 0, issues, lastCheckedAt: Date.now() };
